...
export default OrderSold = ({ navigation }) => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axios.get(
          endpoints.get_orders_status_order_status_pay_true(storeId)
        );
    ...
    <View style={{ backgroundColor: "white" }}>
      <View style={styles.bgNameShop}>
        ...
        <Text style={styles.iconView}>
          <Text style={styles.iconView}>
            {order.order_info.status_pay ? "Đã giao hàng" : "Chưa thanh toán"}
          </Text>
        </Text>
      </View>
      ...
      >
        ...
        <View>
          ...
          >
            ...
            {formatPrice(
              order.order_details.reduce((total, orderDetail) => {
                return total + orderDetail.product.price * orderDetail.quantity;
              }, 0) + order.order_info.shipping_fee
            )}
          </Text>
        </View>
