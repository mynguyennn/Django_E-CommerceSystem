...
const ContentComponent = ({ navigation }) => {
  ...
  const { storeId, productId, productName, productImage } = route.params;
  const [productStatsMonth, setProductStatsMonth] = useState([]);
  const [productStatsQuarter, setProductStatsQuarter] = useState([]);
  const [productStatsYears, setProductStatsYears] = useState([]);
  ...
  const barDataMonth = months.map((month, index) => {
    const monthData = productStatsMonth[index] || { total_revenue: 0 };
    return {
  ...
  const barDataQuarter = quarters.map((quarter, index) => {
    const quarterData = productStatsQuarter[index] || { total_revenue: 0 };
    return {
  ...
  const barDataYear = productStatsYears.map((yearData, index) => ({
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        if (selectedYear) {
          const response = await axios.get(
            endpoints.product_revenue_in_month(storeId),
            {
              params: {
      ...
      try {
        if (selectedYear) {
          const response = await axios.get(
            endpoints.product_revenue_in_quarter(storeId),
            {
              params: {
                product_id: productId,
                year: selectedYear,
      ...
      try {
        if (selectedYear) {
          const response = await axios.get(
            endpoints.product_revenue_in_year(storeId),
            {
              params: {
                product_id: productId,
                year: selectedYear,
          ...
          setProductStatsYears(response.data);
          // console.log("========", response.data);
        }
      } catch (error) {
        console.error("Error fetching data:", error);
    ...
    fetchData();
  }, [selectedYear, storeId, productId]);
              ...
              {months.map((month, index) => (
                ...
                >
                  ...
                  <Text style={{ fontWeight: "500" }}>
                    {productStatsMonth[index]?.total_quantity || 0}sp
                  </Text>

                  <Text style={{ fontSize: 13, marginTop: 10 }}>
                    [{formatPrice(productStatsMonth[index]?.total_revenue || 0)}
                    ]
                  </Text>
                </View>
              ))}
